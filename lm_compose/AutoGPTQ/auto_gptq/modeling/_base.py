...
class BaseGPTQForCausalLM(nn.Module, PushToHubMixin):
    ...
    def post_init(self):
        print("---------------------------------")
        self.model = autogptq_post_init(self.model, use_act_order=self.quantize_config.desc_act)
        ...
        for i in range(len(layers)):
            ...
            for names in inside_layer_modules:
                ...
                for name in subset:
                    ...
                    #copy_w=subset[name].weight.data.clone().detach()
                    ...
                    gptq[name].free()
                    #print("diff",(subset[name].weight.data-copy_w).abs().sum(),subset[name].weight.data.dtype)
            ...
            #self.pseudo_qmodel=copy.deepcopy(self.model)
            del layer
            del gptq
            del layer_inputs
        ...
        for i in range(len(layers)):
            ...
            for names in inside_layer_modules:
                ...
                for name in subset:
                    ...
                    #torch.save(scale,"/scratch-shared/HTJ/Q_Results1/Edit_layer"+str(i+1)+"_"+name+".pt")
                    #print(name,",",scale.shape,subset[name].weight.shape,scale)
                    quantizers[f'{self.layers_block_name}.{i}.{name}'] = (
                        gptq[name].quantizer.to(CPU if force_layer_back_to_cpu else cur_layer_device),
                        move_to_device(scale, CPU if force_layer_back_to_cpu else cur_layer_device),
                        move_to_device(zero, CPU if force_layer_back_to_cpu else cur_layer_device),
    ...
    def from_quantized(
        ...
        **kwargs
        ...
        print("start post init,",quantize_config.desc_act)
