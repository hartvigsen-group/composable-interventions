...
class ConceptEditor:
    ...
    def edit(
        ...
        **kwargs,
        ...
        concept_consistency = (
            kwargs["concept_consistency"]
            if "concept_consistency" in kwargs.keys()
            else False
        )
        ...
        if "pre_edit" in kwargs and kwargs["pre_edit"] is not None:
            ...
            for i, request in tqdm(enumerate(requests)):
                metrics = {
                    "pre": compute_concept_edit_quality(
                        self.model,
                        self.model_name,
                        self.hparams,
                        self.tok,
                ...
                all_metrics[i].update(
                    {
                        ...
                        "time": exec_time,
                        "post": compute_concept_edit_quality(
                            self.model,
                            self.model_name,
                            self.hparams,
                            self.tok,
                            request,
                            self.hparams.device,
                            test_concept_consistency=concept_consistency,
                            P=PMT,
                        ),
                    }
                )
        ...
        if instance_inputs is not None:
            for instance_key in instance_inputs.keys():
                if isinstance(instance_inputs[instance_key]["prompt"], str):
                    instance_inputs[instance_key]["prompt"] = [
                        instance_inputs[instance_key]["prompt"],
                    ]
                for i, request in enumerate(requests):
