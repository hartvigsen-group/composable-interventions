...
class BaseEditor:
    ...
    @classmethod
    def from_hparams(cls, model, hparams: HyperParams):
    ...
    def __init__(
        ...
        if type(self.model_name) is str:
            ...
            if "t5" in self.model_name.lower():
                ...
                if "llama-3" in self.model_name.lower():
                    self.tok = tokenizer = AutoTokenizer.from_pretrained(
                        self.model_name
                    )
                else:
        ...
        if hparams.model_parallel:
            hparams.device = str(self.model.device).split(":")
    ...
    def edit(
        ...
        **kwargs,
        ...
        """
        test_generation = (
            kwargs["test_generation"] if "test_generation" in kwargs.keys() else False
        )
        ...
        if hasattr(self.hparams, "batch_size"):
            assert self.hparams.batch_size == 1, print(
        ...
        if self.alg_name == "FT-Api":
            ...
            start = time()
            edited_model, weights_copy = self.apply_algo(requests, self.hparams)
        ...
        if "pre_edit" in kwargs and kwargs["pre_edit"] is not None:
            ...
            for i, request in tqdm(enumerate(requests)):
                if self.alg_name == "IKE":
                    assert "train_ds" in kwargs.keys(), print(
                ...
                if "metric_kwargs" in kwargs:
                    all_metrics[i].update(
                        compute_sent_metric(
                            ...
                            self.hparams,
                            self.tok,
                            metric_kwargs=kwargs["metric_kwargs"][i],
    ...
    def batch_edit(
        ...
        **kwargs,
        ...
        assert BatchEditor.is_batchable_method(self.alg_name), print(
        ...
        assert hasattr(self.hparams, "batch_size"), print(
    ...
    def edit_dataset(self, ds: Dataset, keep_original_weight=False, verbose=True):
        ...
        assert (
            sum([isinstance(ds, ds_in_dict) for ds_in_dict in DS_DICT.values()]) > 0
        ...
        for record_chunks in tqdm(
            self._chunks(ds, num_edits),
            desc="Editing dataset",
            total=len(ds) / num_edits,
        ):
            start = time()
            edited_model, weights_copy = self.apply_algo(
                self.model,
    ...
    def _prepare_requests(
        ...
        **kwargs,
        ...
        requests = [
            ...
            for prompt, ground_truth_, target_new_ in zip(
                prompts, ground_truth, target_new
            )
        ]
        ...
        if "subject" in kwargs:
            ...
            for prompt_, subject_ in zip(prompts, kwargs["subject"]):
                assert subject_ in prompt_, print(
        ...
        if locality_inputs is not None:
            for locality_key in locality_inputs.keys():
                ...
                assert (
                    len(locality_inputs[locality_key]["prompt"])
                    == len(locality_inputs[locality_key]["ground_truth"])
                    == len(requests)
                ), print("One Edit instance needs one locality input.....")
                ...
                for i, request in enumerate(requests):
                    if locality_inputs[locality_key]["prompt"][i] is not None:
                        request["locality"].update(
                            {
                                locality_key: {
                                    f"prompt": locality_inputs[locality_key]["prompt"][
                                        i
                                    ],
                                    f"ground_truth": locality_inputs[locality_key][
                                        "ground_truth"
                                    ][i],
                                }
                            }
        ...
        if portability_inputs is not None:
            for portability_key in portability_inputs.keys():
                ...
                assert (
                    len(portability_inputs[portability_key]["prompt"])
                    == len(portability_inputs[portability_key]["ground_truth"])
                    == len(requests)
                ), print("One Edit instance needs one portability input.....")
                ...
                for i, request in enumerate(requests):
                    if portability_inputs[portability_key]["prompt"][i] is not None:
                        request["portability"].update(
                            {
                                portability_key: {
                                    "prompt": portability_inputs[portability_key][
                                        "prompt"
                                    ][i],
                                    "ground_truth": portability_inputs[portability_key][
                                        "ground_truth"
                                    ][i],
                                }
                            }
    ...
    def edit_requests(
        self, requests, keep_original_weight=False, verbose=True, **kwargs
    ):
        """
        `prompts`: list or str
            the prompts to edit
        ...
        eval_metric = (
            kwargs["eval_metric"] if "eval_metric" in kwargs.keys() else "exact match"
        )
        if hasattr(self.hparams, "batch_size"):  # For Singleton Editing, bs=1
            self.hparams.batch_size = 1
...
class ModelEditWrapper:
    ...
    def edit(self, *args, **kwargs):
        metrics, self.model, _ = self.base_editor.edit(
            *args, **kwargs
        )  # Replace the internal model with the edited model
        return metrics, self.model  # Return metrics and the edited model

    def batch_edit(self, *args, **kwargs):
        metrics, self.model, _ = self.base_editor.batch_edit(
            *args, **kwargs
        )  # Replace the internal model with the edited model
        return metrics, self.model  # Return metrics and the edited model
    ...
    def __call__(self, *args, **kwargs):
        return self.model(*args, **kwargs)  # Forward the call to the actual model
