        ...
        for lr in "${learning_rates[@]}"; do
            for train_sample_size in "${num_training_samples[@]}"; do
                for retain_weight in "${retain_weights[@]}"; do
                    num_runs=$((num_runs + 1))
                    echo run_exp.sh model_name=$model_name tag="gd_${model_tag}_hparam_search" unlearn=gd interventions=[unlearn] seed=42 wandb=online save_ckpt=False ga_train_size=$train_sample_size ga_lr=$lr ga_retain_weight=$retain_weight
                done
            done
        done
    done
    ...
    for num_batch in "${num_batches[@]}"; do
        for layer in "${layers[@]}"; do
            for alpha in "${alphas[@]}"; do
                num_runs=$((num_runs + 1))
                echo run_exp.sh model_name=$model_name tag="rmu_${model_tag}_hparam_search" unlearn=rmu interventions=[unlearn] seed=42 wandb=online save_ckpt=False rmu_alpha=[$alpha,$alpha] rmu_layer_id=$layer rmu_max_num_batches=$num_batch
            done
        done
    done
fi
