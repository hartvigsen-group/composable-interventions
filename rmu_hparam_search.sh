...
for num_batch in "${num_batches[@]}"; do
    for layer in "${layers[@]}"; do
        for alpha in "${alphas[@]}"; do
            ...
            job_args="tag=rmu_hparam_search model_name=${model} unlearn=rmu interventions=[unlearn] seed=42 wandb=online save_ckpt=False rmu_alpha=[$alpha,$alpha] rmu_max_num_batches=$num_batch rmu_layer_id=$layer rmu_layer_ids=$layer_ids"
