...
for lr in "${learning_rates[@]}"; do
    ...
    # jobs_args+=("tag=ga_mistral_hparam_search unlearn=ga interventions=[unlearn] seed=42 wandb=online save_ckpt=False ga_lr=$lr")
    ...
    for train_sample_size in "${ga_train_sizes[@]}"; do
        # jobs_args+=("tag=ga_mistral_hparam_search unlearn=ga interventions=[unlearn] seed=42 wandb=online save_ckpt=False ga_train_size=$train_sample_size ga_lr=$lr")
